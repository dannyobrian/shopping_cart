...
export const Basket = () => {
    ...
    const itemCount = contents.reduce((acc, curr: StockItem) => {
        if (curr.packSizes) {
    ...
    const parseBasketContents = (): BasketRowLineItem[] =>
    ...
    const BasketRow = ({ item, cumulative = false }: BasketRowLineItem) => {
        ...
        return (
            <div className="basket-row">
                ...
                <div>
                    <Button
                        variant="link"
                        onClick={() => removeItem({ sku: item.sku, size: item.size, qty: 1 }, cumulative)}
                        size="sm"
                    >
                        -
                    </Button>
                ...
                <div>
                    <Button
                        variant="link"
                        onClick={() => addItem({ sku: item.sku, size: item.size, qty: 1 }, cumulative)}
                        size="sm"
                    >
                        +
                    </Button>
    ...
    const BasketContent = () => {
        return (
            <div>
                {contents.length > 0 ? (
                    parseBasketContents().map(row => (
                        <BasketRow key={`basketItem_${row.item.sku}${row.item.size}`} {...row} />
                    ))
                ) : (
